    ...
    try {
      ...
      const progressInterval = setInterval(() => {
        setQueue((prev) => {
          ...
          const newProgress = Math.min(90, current.progress + Math.random() * 15 + 5)
          return prev.map(q => q.id === item.id ? { ...q, progress: newProgress } : q)
        })
      }, 200)
      ...
      if (response.success && response.files.length > 0) {
        ...
        toast({
          title: "上传成功",
          description: `${item.file.name} 已成功上传`
        })
      } else {
    ...
    <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8 py-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>支持拖拽或选择多张图片，批量上传（UI演示）。目标路径将按月份展示：uploads/YYYY/MM</CardDescription>
        </CardHeader>
              ...
              <div key={q.id} className={`flex items-center gap-3 rounded-lg border p-3 ${
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-xs">
                    ...
                    {q.status === "queued" && (
                      <span className="text-gray-400">· 目标路径：uploads/{new Date().getFullYear()}/{String(new Date().getMonth() + 1).padStart(2, "0")}</span>
                    )}
                  </div>
                </div>
